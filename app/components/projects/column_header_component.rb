...
class Projects::ColumnHeaderComponent < ApplicationComponent # rubocop:disable OpenProject/AddPreviewForViewComponent
  ...
  def sort_link(column)
    ...
    link_to({ sortBy: JSON::dump(orders[0..2]) }, title: sort_link_title(column)) do
      column_caption
    end
  end
      ...
      data:
        {
          ...
          "params-from-query-allowed-value": '["query_id", "columns", "filters", "per_page"]',
  ...
  def sort_header_outer_classes(column)
    "generic-table--sort-header-outer_no-highlighting" if lft_column?(column)
  end
